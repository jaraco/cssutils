...
class Tokenizer(object):
    ...
    def getttype(self, t):
      ...
      for check, ntype in _checks:
         if check(t):
           ttype = ntype
           break
         
      if not ttype:
          ttype = self.ttypes.DELIM
    ...
    def addtoken(self, value, ttype=None):
      ...
      if not value:
         return
      # check if known ttype
      if not ttype:
         ttype = self.getttype(value)         
      ...
      # last tokens, if none simple use empty Token to simplify later
      ...
      # ATKEYWORD: combine with last "@" if appropriate (or "@-")
      if u'@' == last.value and ttype == self.ttypes.IDENT:
          keyword = value.lower()
          nobslashkeyword = keyword.replace(u'\\', u'')
          last.type = ats.get(nobslashkeyword, self.ttypes.ATKEYWORD)
          last.value = u'@%s' % keyword
      # @-ATKEYWORD          
      elif u'@' == last2.value and u'-' == last.value and \
         ttype == self.ttypes.IDENT:
          last2.type = ats.get(value.lower(), self.ttypes.ATKEYWORD)
          last2.value = u'@-%s' % value.lower() # put at @
          self.tokens.pop(-1) # remove -

      # IDENT or NUMBER with -
      elif u'-' == last.value and (
        ttype == self.ttypes.IDENT or ttype == self.ttypes.NUMBER):
          last.type = ttype
          last.value = u'-%s' % value        
      
      # DIMENSION: combine num with ident
      elif ttype == self.ttypes.IDENT and last.type == self.ttypes.NUMBER:
          last.type = self.ttypes.DIMENSION
          last.value = u'%s%s' % (last.value, value)

      # HASH: combine # with name
      elif self.getttype(last.value + value) == self.ttypes.HASH:
          last.type = self.ttypes.HASH
          last.value = u'%s%s' % (last.value, value)

      # FUNCTION: combine "(" with preceding if IDENT
      elif u'(' == value and last.type == self.ttypes.IDENT:
          last.type = self.ttypes.FUNCTION
          last.value = u'%s(' % last.value

      # PERCENTAGE: combine '%' with last NUMBER (15.5%)
      elif value == u'%' and last.type == self.ttypes.NUMBER:
      ...
      # IMPORTANT_SYM: combine with last "!" if appropriate "important"
      ## No comments in between!
      elif u'important' == value.lower():
          for i in range(len(self.tokens), 0, -1):
              _t = self.tokens[i-1]
              # check if preceding was "!" => !important and delete nexts
              if _t.value == u'!':
                  _t.type = self.ttypes.IMPORTANT_SYM
                  _t.value = u'!important'
                  del self.tokens[i:]
                  break
    ...
    def tokenize(self, text):
        ...
        def addifnewtoken(t, c):
            ...
            """
            tnt, cnt = self.getttype(t), self.getttype(c)            
            #print t, tnt, c, cnt

            if tnt in (self.ttypes.ATKEYWORD, self.ttypes.IDENT)\
                  or (t and t[-1] == u'-')\
                  and cnt in (self.ttypes.IDENT, self.ttypes.NUMBER):
                # wait for new token "x1..."
                t.append(c)

            elif tnt == self.ttypes.NUMBER and (
                   cnt in (self.ttypes.IDENT, self.ttypes.NUMBER) or
                   c == u'.'
               ):
                # a number with possible . and possible following dim
                # . is always followed by a NUMBER, see caller
                t.append(c)              

            elif cnt and cnt == self.ttypes.DELIM: #in (self.ttypes.ESCAPE, self.ttypes.DELIM):
                # escape always alone
        ...
        while self.text:
            ...
            elif c == u'.' and c2 and c2 in u'0123456789':
                # possible num
                t = addifnewtoken(t, c)
